...
| `barrel-distortion.glsl` | davej, 2015 — GPL-2.0-or-later | **Curvature on its own, and nothing else.** A hardcoded `BARREL_DISTORTION = 0.25` warp with no scaler, pattern or grading, which makes it the only way to price a warp in isolation. `crt-turbo` pays 3.26 ms for curvature with its slider at **zero**, and that is lost hoisting rather than the warp's arithmetic — this row is the arithmetic alone. |
